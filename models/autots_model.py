...
def forecast_single_model(series_df: pd.DataFrame, model_name: str = 'ARIMA', test_fraction: float = 0.2) -> Tuple[float, pd.DataFrame, pd.DataFrame]:
    """
    Run AutoTS with a single model and compute normalized RMSCM.
    Returns (nrmscm, forecast_df, test_df).
    """
    n = len(series_df)
    if n < 30:
        raise ValueError("Insufficient data for AutoTS forecasting")
    ...
    nrmscm = rmse / denom if denom > 0 else rmse
    return nrmscm, forecast_df, test_df